...
Global MostraBordo.b  = 1 ; 1 = Si, 0 = No
Global chkBordo.i
...
Procedure SaveSettings()
  If CreatePreferences(ConfigPath)
    ...
    PreferenceComment(" Mostrabordo: Visualizza bordo nero esterno(1=Si 0=No)")
    ...
    WritePreferenceInteger("MostraBordo", MostraBordo)
...
Procedure LoadSettings()
  ...
  If OpenPreferences(ConfigPath)
    ...
    MostraBordo = ReadPreferenceInteger("MostraBordo", 1)
  ...
  If ListSize(Slices()) = 0
    ...
    AddElement(Slices()) : Slices()\label="SCRNSHOT" : Slices()\cmd="snippingtool.exe"     : Slices()\color=$FF8000
...
Procedure CreateMenuImage(HoverIdx.i = -1)
  ...
  ; --- CENTRO E RAGGIO ---
  Protected centro.f = (WinSize - 1) / 2.0
  Protected rEsterno.f = centro - 1.5 ; Leggermente rientrato per l'antialiasing
  ...
  If StartDrawing(ImageOutput(MenuImage))
    ...
    ForEach Slices()
      ...
      If i = HoverIdx : drawColor = RGB(Red(drawColor)+50, Green(drawColor)+50, Blue(drawColor)+50) : EndIf

      rad = Slices()\startAngle
      While rad <= Slices()\endAngle
        LineXY(centro, centro, centro + (centro * Cos(Radian(rad-90))), centro + (centro * Sin(Radian(rad-90))), drawColor)
        rad + (EffettoLoFi * 0.5)
      Wend
      i + 1
    Next

    ; 3. RAGGI SEPARATORI (Neri)
    i = 0
    ForEach Slices()
      Protected angolo.f = Radian((i * angleStep) - 90)
      LineXY(centro, centro, centro + rEsterno * Cos(angolo), centro + rEsterno * Sin(angolo), $000000)
      i + 1
    Next
    ...
    ForEach Slices()
      ...
      If i = HoverIdx : textOffset = 5.0 : EndIf
    ...
    ; 5. BUCO CENTRALE (Semplice cerchio nero di fondo)
  ...
  ;Bordo esterno
   If mostrabordo
  If StartVectorDrawing(ImageVectorOutput(MenuImage))
   
      ; Bordo Esterno
      AddPathCircle(centro, centro, rEsterno)
      VectorSourceColor(RGBA(0, 0, 0, 255)) ; Nero pieno
      StrokePath(2) ; Spessore 2.5 pixel, liscio e netto
      
      ; Bordo Buco Centrale
      ;AddPathCircle(centro, centro, RaggioBuco)
      ;VectorSourceColor(RGBA(0, 0, 0, 255))
      ;StrokePath(1.5) ; Un po' più sottile per il buco    
      StopVectorDrawing()
    EndIf
  EndIf
EndProcedure
...
Procedure OpenSettingsWin()
  ...
  If OpenWindow(#WinSettings, 0, 0, 620, 620, "MouseLauncher - Settings", #PB_Window_SystemMenu | #PB_Window_ScreenCentered)
    StickyWindow(#WinSettings, #True)   
    SetWindowColor(#WinSettings, #PB_Default) 

    FrameGadget(#PB_Any, 10, 5, 600, 270, " Impostazioni Generali ")
    ...
    y + 26 : TextGadget(#PB_Any, 25, y+3, 80, 20, "Mostrabordo:")
    Protected G_mostrabrd = CheckBoxGadget(#PB_Any, 110, y, 150, 22, "Mostra bordo")
    SetGadgetState(G_mostrabrd, Mostrabordo) 
    ; --- Elenco ---
    y + 30
    ...
    y+40
    ButtonGadget(#BtnSave, 15, y, 590, 40, "SALVA E CHIUDI", #PB_Button_Default)
        ...
        Case #PB_Event_Gadget
          ...
          Select gd
            ...
            Case #BtnSave             
              ...
              MostraBordo = GetGadgetState(G_mostrabrd)
              Protected stile = #PB_Font_HighQuality
              If FontBold : stile | #PB_Font_Bold : EndIf
...
; EnableXP
; DPIAware
; UseIcon = mouselauncher1.ico
; Executable = mouselauncher2.exe